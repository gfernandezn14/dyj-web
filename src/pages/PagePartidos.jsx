  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 0, maxWidth: 1200 }}>
      {DYJ_DATA.resultadosRecientes.map((r, i) => {
        const colorBar = r.resultado === 'V' ? 'var(--dyj-gold)' : r.resultado === 'D' ? 'var(--dyj-red-text)' : 'rgba(0,0,0,0.3)';
        return (
  ...
  return (
    <div style={{ marginBottom: 60 }}>
      <div style={{
        ...
        <div className="display" style={{ fontSize: 56, lineHeight: 1, color: 'var(--dyj-red-text)' }}>Grupo {grupo}</div>
      ...
      {equipos.map((t) => {
        ...
        return (
          <div key={t.pos} style={{
            ...
            padding: '20px 16px',
            alignItems: 'center',
            background: esDyj ? 'var(--dyj-red-text)' : 'transparent',
            ...
            <span className="display" style={{ fontSize: 28, textAlign: 'right', color: esDyj ? 'var(--dyj-gold)' : 'var(--dyj-red-text)' }}>{t.pts}</span>
          </div>
        );
      })}
    </div>
