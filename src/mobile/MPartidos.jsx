  ...
  return (
    <div style={{ marginBottom: 36 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, padding: '0 4px 14px', flexWrap: 'wrap' }}>
        <div className="display" style={{ fontSize: 36, lineHeight: 1, color: 'var(--dyj-red-text)' }}>
          Grupo {grupo}
        </div>
          ...
          {equipos.map((t) => {
            ...
            return (
              <div key={t.pos} style={{
                ...
                <span style={{ fontFamily: 'var(--font-display)', fontSize: 22, textAlign: 'right', color: esDyj ? 'var(--dyj-gold)' : 'var(--dyj-red-text)' }}>{t.pts}</span>
              </div>
            );
          })}
        </div>
      ...
      <section className="m-sec" style={{ paddingTop: 32 }}>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          {tab === 'resultados' && d.resultadosRecientes.map((r, i) => {
            const accentBg = r.resultado === 'V' ? 'var(--dyj-gold)' : r.resultado === 'D' ? 'var(--dyj-red-text)' : 'rgba(0,0,0,0.3)';
